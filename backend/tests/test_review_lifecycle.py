...
class ReviewLifecycleTests(ClientDashboardSourcesTests):
    async def test_finding_action_recurrence_and_tenant_links(self):
        ...
        completed = await self.client.post("/api/reviews/tabletop/complete", json={"spawn_next": True})
        ...
        await self.client.patch(f'/api/findings/{finding["finding_id"]}', json={"status": "closed"})
