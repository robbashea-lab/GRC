...
(async()=>{
  ...
  try{
    ...
    for(const [key,id,type] of cases.filter(c=>!process.argv[2]||c[0]===process.argv[2])){
      ...
      const search=page.getByRole('textbox',{name:/^Search /});await search.fill(id);await page.getByTestId(type+'-'+id).getByRole('button').first().click();await expect(drawer()).toBeVisible();await page.keyboard.press('Escape');await expect(drawer()).toHaveCount(0);await expect(search).toHaveValue(id);await search.fill('');
