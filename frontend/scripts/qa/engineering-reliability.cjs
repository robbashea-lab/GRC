...
(async()=>{
  ...
  try {
    ...
    const identity=await page.evaluate(()=>{
      const db=JSON.parse(sessionStorage.getItem('grc_interactive_demo_v2'));
    ...
    await page.evaluate(id=>{
      const key='grc_interactive_demo_v2',db=JSON.parse(sessionStorage.getItem(key)),row=db.findings.find(r=>r.finding_id===id);
    ...
    assert.equal(await page.evaluate(id=>JSON.parse(sessionStorage.getItem('grc_interactive_demo_v2')).findings.find(r=>r.finding_id===id).title,identity.id),'Newer saved finding');
