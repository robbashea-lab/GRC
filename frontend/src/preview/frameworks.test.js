...
test('new intake begins with programs; catalog is IG1 only; cadence warning separates automation',()=>{
  ...
  expect(FRAMEWORKS.filter(f=>f.implemented).map(f=>f.key)).toEqual(['hipaa','cis-ig1','iso-27001','soc-2']);
...
test.each([[],['cis-ig1'],['hipaa'],['iso-27001','soc-2'],['cis-ig1','hipaa','nist-csf-2']])('selection %j creates only implemented requirements and reviews',async(...args)=>{
  ...
  expect(await get('reviews')).toHaveLength(programs.includes('cis-ig1')?(programs.includes('hipaa')?18:12):programs.includes('hipaa')?8:programs.includes('iso-27001')?13:0);
  ...
  for(const key of ['hipaa','nist-csf-2','iso-27001','cmmc','soc-2']){
    const shell=await get('frameworks/'+key),count=programs.includes(key)?({hipaa:76,'iso-27001':123,'soc-2':33}[key]||0):0;expect(shell.assessments).toHaveLength(count);expect(shell.definitions).toHaveLength(count);
  }
});
