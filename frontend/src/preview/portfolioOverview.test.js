...
test('all five Demo portfolios reconcile to source records and remain read-only',()=>{
...
test('archived clients are opt-in; empty scoped assignment does not fall back to all',()=>{
  ...
  expect(portfolio(db,false).clients).toHaveLength(4);
  expect(portfolio(db,true).clients).toHaveLength(5);
