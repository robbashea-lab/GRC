...
export const SOURCE_TYPES = { manual: 'Manual / Internal', review: 'Review', finding: 'Finding', risk: 'Risk', vendor: 'Vendor', policy: 'Policy', audit: 'Audit / Assessment', incident:'Incident (context only)', other:'Other (context only)' };
...
export const actionTitle = row => row.title_generated===true&&row.title?.startsWith('Remediate: ')?row.title.slice(11):row.title;
...
export function taskSource(row, records={}) {
  const type=row.source_type || (SOURCE_TYPES[row.source]?row.source:null) || ['finding',...Object.keys(SOURCE_RECORDS)].find(t=>row[SOURCE_RECORDS[t][1]]) || (row.source ? 'legacy' : 'unknown');
  const [kind,key]=SOURCE_RECORDS[type]||[];
  const id=row[key] || row.source_id;
