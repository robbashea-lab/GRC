import {riskStatus} from './riskRegister';
import { SOURCE_TYPES, actionStatus, actionPriority, actionMatches } from './actionItems';
...
export function tableColumns(module, { rows=[], users=[], clients=[], programs={} } = {}) {
  ...
  if (module === 'risk-register') return [text('title','Risk'),select('category','Category'),numeric('likelihood_score','Likelihood',true),numeric('impact_score','Impact',true),numeric('risk_score','Score'),severity('risk_level','Level'),person('owner_id'),select('status','Status',{value:r=>r.status||'open',labelValue:riskStatus}),date('last_reviewed','Last reviewed','history','Never Reviewed'),date('next_review','Next Review','future','No Review Scheduled')];
  ...
  if (module === 'action-items') return [text('title','Action Item'),{...severity('priority','Priority'),labelValue:actionPriority},person('owner_id'),{...date('due_date','Due','due','No Due Date'),matches:(r,v,now)=>v==='overdue'?actionMatches(r,'overdue',now):dateMatches(r.due_date,v,now)},select('status','Status',{labelValue:actionStatus}),select('source_type','Source',{labelValue:v=>SOURCE_TYPES[v]|| (v==='legacy'?'Legacy source':'Not recorded')})];
  ...
  if (module === 'client-management') return [text('name','Client'),select('industry','Industry'),lead('assigned_owner_id'),select('program_status','Program Status',{value:r=>programs[r.client_id]}),select('status','Client Status',{value:r=>r.status||'active'})];
  if (module === 'portfolio') return [text('name','Client'),lead('grc_lead_id'),
    select('frameworks','Frameworks',{sortable:false,emptyLabel:'No frameworks selected',value:r=>(r.frameworks||[]).map(f=>f.key),optionsOnly:true,
      options:[...new Map(rows.flatMap(r=>(r.frameworks||[]).map(f=>[f.key,{value:f.key,label:f.label}]))).values()]}),
    ...['past_due','due_30d','critical_high_issues','significant_risks','unassigned'].map(key=>({...numeric(key,({past_due:'Past Due',critical_high_issues:'Critical / High',significant_risks:'Significant Risks',unassigned:'Unassigned',due_30d:'Due ≤30d'})[key]),filter:true,optionsOnly:true,emptyLabel:null,options:[{value:'some',label:'Has items'},{value:'none',label:'No items'}],matches:(r,v)=>v==='some'?r[key]>0:!r[key]})),
    {...date('last_activity','Last Activity','history','No recorded activity'),value:r=>r.last_activity?.at}];
