...
test('shared system adds no decorative animation, blur or promoted layers', () => {
...
test.each(['text-primary','text-secondary','text-muted','text-help'])('%s meets 4.5:1 across approved surfaces', text => {
...
test('search outline remains distinguishable from its fill',()=>{
  expect(contrast(token('input-border'),token('bg-surface'))).toBeGreaterThanOrEqual(3);
});
...
test('four text levels are ordered and active boundaries remain distinguishable', () => {
  ...
  expect(contrast(token('control-active-border'),token('bg-surface'))).toBeGreaterThanOrEqual(3);
  expect(contrast(token('selected-rail'),token('selected-bg'))).toBeGreaterThanOrEqual(3);
  expect(contrast(token('text-muted'),token('selected-bg'))).toBeGreaterThanOrEqual(4.5);
});

test('approved light canvas and charcoal navigation are the single shared palette', () => {
  const palette = postcss.parse(css).nodes.find(n => n.selector === ':root');
  expect(palette.nodes.find(n => n.prop === 'color-scheme').value).toBe('light');
  expect(token('bg-app')).toBe('#F7F9FA');
  expect(token('bg-surface')).toBe('#FFFFFF');
  expect(token('bg-tablehead')).toBe('#F1F3F6');
  expect(token('sidebar-bg')).toBe('#293039');
