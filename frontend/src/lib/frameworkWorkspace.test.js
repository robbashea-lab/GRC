...
test('operational projection follows relationships without leaking other clients',()=>{
  ...
  const work=assessmentWork(row,{reviews:[{client_id:'a',review_id:'r',status:'upcoming',due_date:'2026-01-01'},{client_id:'b',review_id:'r',status:'upcoming',due_date:'2026-01-01'}],findings:[{client_id:'a',finding_id:'f',review_id:'r',status:'open'}],tasks:[{client_id:'a',task_id:'t',finding_id:'f',due_date:'2026-01-01',status:'open'}]},'2026-01-02');
  expect(work).toEqual({review_ids:['r'],finding_ids:['f'],open_findings:1,overdue_reviews:1,overdue_actions:1});
});
