    ...
    <table>
      ...
      <tbody>{rows.map(r=>{
        ...
        return <tr key={r.framework_assessment_id} data-testid={'requirement-'+r.definition_id}>
          ...
          <td data-label="Assessment"><CisStatusPill status={r.status}/></td>
          ...
          <td data-label="Last assessed">{fresh.state==='stale'?<Tag tone="moderate">{fresh.label}</Tag>:<span className="text-xs text-ink-secondary">{fresh.state==='never'?'Never':r.last_assessed.slice(0,10)}</span>}</td>
          <td data-label="Remediation"><span className="cis-results-remediation">{w.overdue_actions>0&&<Tag tone="critical">{w.overdue_actions} overdue</Tag>}{directFindings(w)>0&&<Tag tone="neutral">{directFindings(w)} open Finding{directFindings(w)===1?'':'s'}</Tag>}{gapUntracked(r)&&<Tag tone="critical">Gap not tracked</Tag>}{!w.overdue_actions&&!directFindings(w)&&!gapUntracked(r)&&<span className="text-xs text-ink-muted">—</span>}</span></td>
        </tr>;})}</tbody>
    </table>
  </div>;
}
