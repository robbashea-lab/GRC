...
export default function VendorGovernancePanel({tab,record,form,setForm,canWrite,isAdmin,users,reviews,tasks,risks,evidence,openRecord,uploadFiles,downloadEv,onSaved}) {
  ...
  if(tab==='risks_tab') return <div className="space-y-3">{write&&<><Button size="sm" onClick={()=>openRecord({kind:'risks',record:null,initialValues:{source_type:'vendor',source_id:record.vendor_id,vendor_id:record.vendor_id}})}>Create Risk</Button>
    <Select value="" onValueChange={async id=>{try{await api.patch('/vendors/'+record.vendor_id,{related_risk_ids:[...new Set([...(record.related_risk_ids||[]),id])]});onSaved();toast.success('Risk linked');}catch(e){toast.error(formatError(e));}}}><SelectTrigger aria-label="Link existing Risk"><SelectValue placeholder="Link existing Risk"/></SelectTrigger><SelectContent>{riskOptions.filter(r=>!risks.some(x=>x.risk_id===r.risk_id)).map(r=><SelectItem key={r.risk_id} value={r.risk_id}>{r.display_id} · {r.title}</SelectItem>)}</SelectContent></Select></>}
