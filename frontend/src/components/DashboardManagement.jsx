...
export default function DashboardManagement({clientId,posture,programs=[],framework,onOpen,loadDetail,Table,reference=false}) {
  ...
  if(reference&&!framework)return <div className="page-content space-y-6" data-testid="management-dashboard">
    ...
    <div className="dash-primary">
      <Panel title="Highest priority items" description="Ranked by lateness, then severity. Open an item to work it in its source record.">
        {posture.priority.length?<Table items={posture.priority.slice(0,6)} onOpen={onOpen}/>:<p className="text-sm text-ink-muted">No items require immediate attention right now.</p>}
        {posture.priority.length>6&&<button type="button" onClick={()=>show('All priority items',posture.priority,'priority')} className="text-sm text-link hover:text-link-hover">View all {posture.totals?.priority??posture.priority.length}</button>}
      </Panel>
      <DashboardPrograms clientId={clientId} programs={programs} onOpen={onOpen} reference/>
    </div>
    <div><h2 className="text-sm font-heading font-semibold text-ink-primary mb-2">Program health</h2><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <Panel title="GRC work status" description="Current obligations by due state."><Distribution groups={posture.buckets} onOpen={show} /></Panel>
      <Panel title="Risk posture" description="Active risks by level."><Distribution groups={posture.riskLevels} onOpen={show} /><button className="flex justify-between gap-3 w-full border-t border-line pt-3 text-sm text-link" onClick={()=>show('Accepted Risks',posture.acceptedRisks||[],'acceptedRisks')}><span>Accepted risks</span><span className="font-mono">{posture.totals?.acceptedRisks??posture.acceptedRisks?.length??0}</span></button></Panel>
      <Panel title="Third-party oversight" description="Reviews due within 30 days; assurance and contracts use each vendor's lead window."><div className="space-y-1">{posture.vendorHealth.map(g=>{const n=g.total??g.items.length;return <button key={g.key} type="button" onClick={()=>show(g.label,g.items,g.key)} className="w-full flex justify-between gap-3 rounded px-1 py-1 text-sm text-left text-ink-secondary hover:bg-surface-subtle hover:text-ink-primary focus-visible:ring-2 focus-visible:ring-ring" aria-label={`${g.label}: ${n} items`}><span>{g.label}</span><span className={`font-mono font-medium ${n&&['vendorReviewsPast','assurance','missingAssurance'].includes(g.key)?'text-semantic-duesoon-text':'text-ink-primary'}`}>{n}</span></button>;})}</div></Panel>
    </div></div>
    {drawer}
  </div>;
