...
export default function AIGovernance(){
  ...
  if(!currentClientId)return <p className="p-8">Select a client.</p>;
  return <div><PageHeader title="AI Governance" subtitle="AI Systems & Use Cases Register · Internal governance screening, not legal classification" action={writable&&data?.intake.usage!=='no'?<Button onClick={()=>setSelected({})}>Add AI System</Button>:null}/><div className="p-4 md:p-8 space-y-4">{error?<p role="alert">{error} <button onClick={()=>setRevision(n=>n+1)}>Retry</button></p>:!data?<p>Loading AI Governance…</p>:<>
