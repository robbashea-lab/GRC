...
const TONE={yes:'bg-semantic-success-bg text-semantic-success border-semantic-success-border',no:'bg-semantic-critical-bg text-semantic-critical border-semantic-critical-border',unsure:'bg-semantic-duesoon-bg text-semantic-duesoon-text border-semantic-duesoon-border',applies:'bg-semantic-success-bg text-semantic-success border-semantic-success-border',does_not_apply:'bg-surface-subtle text-ink-secondary border-line-strong'};
...
export default function Onboarding() {
  ...
  return <div><PageHeader title="GRC Program Onboarding" subtitle="Establish the client's initial GRC baseline: reported policies, applicable requirements, and review areas to prepare."/>
    ...
    <div className="p-8 max-w-6xl"><ol className="flex flex-wrap items-center gap-3 mb-6 text-xs font-medium" data-testid="onboarding-stepper">{STEPS.map((name,i)=><li key={name} className={`flex items-center gap-2 ${i===step?'text-ink-primary':'text-ink-help'}`}><span className={`h-5 w-5 rounded-full border flex items-center justify-center text-xs ${i===step?'border-line-strong bg-primary text-primary-foreground':'border-line-strong'}`}>{i+1}</span>{name}{i<3&&<ChevronRight className="h-3 w-3"/>}</li>)}</ol>
    <div className="bg-surface-card border border-line rounded-lg p-6"><h2 className="text-base font-medium mb-2">{STEPS[step]}</h2>
