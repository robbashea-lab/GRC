  ...
  return <div><PageHeader title="GRC Program Onboarding" subtitle="Select applicable programs, capture governance documents, and confirm the actual Review program."/>
    ...
    <section className="border border-line bg-surface-card rounded-lg p-6"><h2 className="text-base font-semibold mb-4">{STEPS[step]}</h2>
      {step===0&&<div className="space-y-5"><p className="text-sm text-ink-muted">Select relevant programs. A selection records applicability—not certification. General GRC onboarding does not require a framework.</p>
        <div className="grid md:grid-cols-2 gap-3">{FRAMEWORKS.map(f=><label key={f.key} className="onboarding-program-option border border-line rounded p-4"><span className="block text-sm font-medium">{f.name}</span><select aria-label={f.name} className="mt-2 border border-line rounded bg-surface-card p-2 text-sm" value={state.requirements[f.key]} onChange={e=>update({...state,requirements:{...state.requirements,[f.key]:e.target.value}})}>{APPLICABILITY.map(([v,label])=><option key={v} value={v}>{label}</option>)}</select><span className="block text-xs text-ink-muted mt-2">{f.implemented?'Safeguard assessments and mapped Reviews available.':'Program workspace only. Detailed mapping is not yet configured.'}</span></label>)}</div>
        <AIIntake key={currentClientId} clientId={currentClientId} canWrite={canRun}/>
      </div>}
      {step===1&&<div className="space-y-3"><p className="text-sm text-ink-muted">Reported existence is not verification. CIS context is shown only when selected; other programs add no Policy mappings.</p><p role="status" className="text-sm font-medium">{catalog.policies.length-missing.length} of {catalog.policies.length} answered · {missing.length ? `${missing.length} responses still required` : 'All required responses recorded'}</p><p className="text-xs text-ink-secondary">Each response is required. Unsure is valid; owners, documents and schedules can be completed later.</p>{validation&&!!missing.length&&<p role="alert" className="text-sm text-semantic-critical">Choose Yes, No or Unsure for each unanswered Policy before continuing.</p>}{catalog.policies.map(p=>{
