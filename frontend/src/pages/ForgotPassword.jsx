      ...
      <div className="w-full max-w-sm bg-surface-card text-ink-primary rounded-xl border border-line p-6">
        ...
        {sent ? (
          <div className="space-y-3">
            <div className="rounded-md border border-semantic-success-border bg-semantic-success-bg text-semantic-success text-sm p-3" data-testid="forgot-sent-message">
              If <span className="font-medium">{email}</span> is registered, a reset link is on its way. It expires in 2 hours.
            </div>
            <Link to="/login" className="text-xs text-link hover:text-link-hover underline">Back to sign in</Link>
          </div>
        ) : (
          <form onSubmit={submit} className="space-y-3">
            ...
            <Button data-testid="forgot-submit" type="submit" disabled={loading} className="w-full bg-brand-charcoal hover:bg-brand-charcoal-hover">
              {loading ? "…" : "Send reset link"}
            </Button>
            <div className="text-xs text-ink-muted text-center pt-2">
              <Link to="/login" className="text-link hover:text-link-hover underline">Back to sign in</Link>
            </div>
          </form>
        )}
      </div>
