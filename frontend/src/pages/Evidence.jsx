...
function EvidenceWorkspace(){
  ...
  return <div><PageHeader title="Evidence Library" subtitle={`${currentClient?.name||''} · Upload where work happens. Find it here with its history intact.`} action={canWrite&&<Button onClick={()=>setAdding(true)}>Add Evidence</Button>}/><div className="page-content space-y-4">
    ...
    {isBrawndoReference(currentClientId,user)&&<div className="register-signals !px-0 !pt-0" role="group" aria-label="Evidence views">{[['evidence_date','older12','Evidence older than 12 months','moderate'],['created_at','last30','Added in last 30 days','info']].map(([key,range,label,tone])=>{const on=(table.state.filters?.[key]||[]).includes(range);
      return <button key={range} type="button" aria-pressed={on} className={`register-signal is-${tone}`} onClick={()=>{setAll(true);setArea(null);setSource(null);setSelectedSet(null);table.setFilter(key,on?[]:[range]);}}><span className="register-signal-label">{label}</span></button>;})}</div>}
